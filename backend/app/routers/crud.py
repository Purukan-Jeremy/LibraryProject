...
def update_book(db: Session, book_id: int, book: schemas.BookCreate):
    ...
    new_author_name = book.author_name

    # 4. Tangani perubahan Author
    if new_author_name:
        db_author = db.query(models.Author).filter(models.Author.author_name == new_author_name).first()
        if not db_author:
    ...
    db_book.file_pdf = book.file_pdf
    ...
    # 6. Bersihkan author lama jika tidak punya buku lain
    if old_author_name and old_author_name != new_author_name:
        other_books = db.query(models.Book).filter(models.Book.author_name == old_author_name).first()
        if not other_books:
            db_old_author = db.query(models.Author).filter(models.Author.author_name == old_author_name).first()
            if db_old_author:
